# `doichain.conf` Configuration File

The configuration file is used by `doichaind`, `doichain-qt` and `doichain-cli`.

All command-line options (except for `-?`, `-help`, `-version` and `-conf`) may be specified in a configuration file, and all configuration file options (except for `includeconf`) may also be specified on the command line. Command-line options override values set in the configuration file and configuration file options override values set in the GUI.

Changes to the configuration file while `doichaind` or `doichain-qt` is running only take effect after restarting.
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `doichain.conf` and it is located in the Doichain data directory, but both the Doichain data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.

The `includeconf=<file>` option in the `doichain.conf` file can be used to include additional configuration files.

### Default configuration file locations

Operating System | Data Directory | Example Path
-- | -- | --
Windows | `%LOCALAPPDATA%\Doichain\` | `C:\Users\username\AppData\Local\Doichain\doichain.conf`
Linux | `$HOME/.doichain/` | `/home/username/.doichain/doichain.conf`
macOS | `$HOME/Library/Application Support/Doichain/` | `/Users/username/Library/Application Support/Doichain/doichain.conf`
...
The output is placed under `share/examples/doichain.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/doichain.conf ~/.doichain
```
