...
name_doi ()
{
  ...
  return RPCHelpMan ("name_doi",
      ...
      RPCResult {RPCResult::Type::STR_HEX, "", "the transaction ID"},
      RPCExamples {
          HelpExampleCli ("name_doi", "\"mydoi\", \"new-value\"")
        + HelpExampleRpc ("name_doi", "\"mydoi\", \"new-value\"")
      },
      [&] (const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
  std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest (request);
  ...
  /* For DOI operations, we can either create a new DOI or update an existing one.
     First check if there are pending DOI operations in the mempool. */
  const unsigned chainLimit = gArgs.GetIntArg ("-limitnamechains",
                                               DEFAULT_NAME_CHAIN_LIMIT);
  COutPoint outp;
  ...
  /* If no pending operations, check if the DOI name already exists */
  if (outp.IsNull ())
    {
      LOCK (cs_main);
      CNameData oldData;
      const auto& coinsTip = chainman.ActiveChainstate ().CoinsTip ();
      if (coinsTip.GetName (name, oldData) && !oldData.isExpired (chainman.ActiveHeight ()))
        {
          /* Existing DOI name - this will be an update */
          outp = oldData.getUpdateOutpoint ();
        }
      /* If outp is still null, this is a new DOI registration */
    }
  ...
  if (!outp.IsNull ())
    {
      /* This is an update of an existing DOI name */
      const CTxIn txIn(outp);
      txidVal = SendNameOutput (request, *pwallet, destHelper.getDest (), nameOp,
                               &txIn, options);
    }
  else
    {
      /* This is a new DOI registration */
      txidVal = SendNameOutput (request, *pwallet, destHelper.getDest (), nameOp,
                               nullptr, options);
    }
