...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Doichain
    //   new: C:\Users\Username\AppData\Local\Doichain
    // macOS: ~/Library/Application Support/Doichain
    // Unix-like: ~/.doichain
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Doichain";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Doichain";
#else
    fs::path pathRet;
